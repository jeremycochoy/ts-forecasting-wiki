...
- **[CPC](../papers/cpc.md)** (van den Oord, Li, Vinyals, DeepMind 2018) — strided CNN encoder + GRU aggregator + InfoNCE; 64.6% LibriSpeech phone accuracy from a linear probe vs. 74.6% supervised. Multi-domain (audio, vision, text, RL) but the audio setup is the prototype TS application.
...
- **[TS2Vec](../papers/ts2vec.md)** (Yue et al., AAAI 2022) — 10-block dilated CNN encoder, contextual consistency under timestamp masking and random cropping, hierarchical contrast (instance-wise + temporal, recursive max-pool). +2.4% over T-Loss on UCR-125 and +3.0% on UEA-29 with 40× shorter training.
...
- **[CPC](../papers/cpc.md)** — the foundational sequential-contrastive recipe; explicit GRU aggregator + InfoNCE, the architecture "GRU + contrastive loss" most directly maps to.
- **[TS2Vec](../papers/ts2vec.md)** — top-cited TS-specific contrastive paper, hierarchical and timestamp-level; the standard reference for representation-learning baselines in TS classification leaderboards (UCR/UEA).

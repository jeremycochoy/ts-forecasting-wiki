...
Methodology paper — **no neural architecture component.** This is a keyword-selection procedure for nowcasting regression, not a model. The downstream forecaster is a plain AR(2) regression augmented with a single Google Trends keyword regressor; the `(L, d_model, d_ff, heads, d_kv, params, patch, context)` tuple does not apply. Empirical scope: 10 candidate keywords on UK unemployment / Claimant count, monthly 2004:M1–2012:M4, refit on a single CPU.
